...
public partial class StartExam : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            try
            {
                ...
                BindData((int)ViewState["QuestionIndex"]);
    ...
    //private void BindData(int questionIndex , string subjectcode)
    //{
    //    SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["OnlineExam"].ConnectionString.ToString());
    //    conn.Open();
    //    DataTable dt = new DataTable();
    //    string SqlQuery = @"SELECT * FROM dbo.OLN_QuestionPaperSeries INNER JOIN dbo.OLN_Subjects 
    //                   ON dbo.OLN_QuestionPaperSeries.subjectcode = dbo.OLN_Subjects.subjectcode 
    //                   AND dbo.OLN_QuestionPaperSeries.ic = dbo.OLN_Subjects.ic 
    //                   AND dbo.OLN_QuestionPaperSeries.sc = dbo.OLN_Subjects.sc 
    //                   WHERE OLN_QuestionPaperSeries.ic=@ic 
    //                   AND OLN_QuestionPaperSeries.sc=@sc 
    //                   AND OLN_QuestionPaperSeries.examcode=@examcode 
    //                   AND OLN_QuestionPaperSeries.subjectcode=@subjectcode  
    //                   AND OLN_QuestionPaperSeries.SeriesCode='A' 
    //                   ORDER BY OLN_QuestionPaperSeries.Seriesquestion_code";
    ...
    private void BindData(int SelectedPage)
    {
        ...
        string SqlQuery = @"SELECT * FROM dbo.OLN_QuestionPaperSeries INNER JOIN dbo.OLN_Subjects 
                      ...
                      " WHERE OLN_QuestionPaperSeries.ic = '" + IC + "'  " +
                       "  AND OLN_QuestionPaperSeries.sc = '" + SC + "'  " +
                       "  AND OLN_QuestionPaperSeries.examcode = '" + examcode + "'  " +
                        " AND OLN_QuestionPaperSeries.subjectcode = '105'  " +
                       "  AND OLN_QuestionPaperSeries.SeriesCode = 'A' " +
 " ORDER BY OLN_QuestionPaperSeries.Seriesquestion_code";
        SqlCommand cmd = new SqlCommand(SqlQuery);
        ...
        if (ViewState["PageNumber"] != null)
            pds.CurrentPageIndex = Convert.ToInt32(ViewState["PageNumber"]);
        else
            pds.CurrentPageIndex = 0;
        if (pds.PageCount > 1)
        {
            Rep_Exam.Visible = true;
            ArrayList pages = new ArrayList();
            for (int i = 0; i < pds.PageCount; i++)
                pages.Add((i + 1).ToString());
            Rep_Exam.DataSource = pages;
            Rep_Exam.DataBind();
        }
        else
        {
            Rep_Exam.Visible = false;
        }
    }
